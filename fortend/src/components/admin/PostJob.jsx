...
const PostJob = () => {
    ...
    const changeEventHandler = (e) => {
        ...
        // Convert salary and position to numbers if the field matches
        if (name === 'salary' || name === 'position') {
            const numberValue = name === 'salary' ? parseFloat(value) : parseInt(value, 10);
            setInput({ ...input, [name]: isNaN(numberValue) ? "" : numberValue });
        } else {
    ...
    const submitHandler = async (e) => {
        ...
        try {
            setLoading(true);
            // Basic validation
            if (!input.title || !input.salary || !input.companyId) {
